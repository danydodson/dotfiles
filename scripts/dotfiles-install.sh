...
setup_gitconfig () {
  if ! [ -f specific/git/config+git+config-local.symlink ]; then
    ...
    sed \
      ...
      specific/git/config+git+config-local.symlink.example > specific/git/config+git+config-local.symlink

...
BINARIES=(ack asciinema bat bc bun curl direnv exiftool fd findutils flyctl fzf gh git go grep heroku htop jq lesspipe luarocks neofetch nvm openjdk perl pipenv pnpm prettyping pyenv-virtualenv ranger rhash ripgrep ruby shellcheck starship stylua tldr wdc web-ext wget 'yt-dlp' zlib zsh-autosuggestions zsh-completions zsh-autocomplete zsh-history-substring-search zsh-syntax-highlighting)
...
CASKS=('1password-cli' apparency cheatsheet docker ngrok neovim openinterminal postman rectangle slack spotify 'suspicious-package' 'tor-browser' transmission vlc)

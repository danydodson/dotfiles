...
// I revisited the rain effect I did for another shader. This one is better in
// multiple ways:
...
vec4 N14(float t) {
  return fract(sin(t * vec4(123., 1024., 1456., 264.)) *
...
vec2 DropLayer2(vec2 uv, float t) {
  ...
  float droplets =
      max(0., (sin(y * (1. - y) * 120.) - st.y)) * trail2 * trailFront * n.z;
  y = fract(y * 10.) + (st.y - .5);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  col *=
      1. + lightning * fade * mix(1., .1, story * story); // composite lightning

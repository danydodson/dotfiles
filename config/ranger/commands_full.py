...
class shell(Command):
    ...
    def tab(self, tabnum):
        from ranger.ext.get_executables import get_executables  # type: ignore
        if self.arg(1) and self.arg(1)[0] == '-':
            command = self.rest(2)
        else:
            command = self.rest(1)
...
class open_with(Command):
    ...
    @staticmethod
    def _is_flags(arg):
        from ranger.core.runner import ALLOWED_FLAGS  # type: ignore
...
class set_(Command):
    ...
    def tab(self, tabnum):  # pylint: disable=too-many-return-statements
        from ranger.gui.colorscheme import get_all_colorschemes  # type: ignore
...
class terminal(Command):
    ...
    def execute(self):
        from ranger.ext.get_executables import get_term  # type: ignore
...
class load_copy_buffer(Command):
    ...
    def execute(self):
        import sys
        from ranger.container.file import File  # type: ignore
...
class bulkrename(Command):
    ...
    def execute(self):  # pylint: disable=too-many-locals,too-many-statements
        ...
        import tempfile
        from ranger.container.file import File  # type: ignore
        from ranger.ext.shell_escape import shell_escape as esc  # type: ignore
...
class stage(Command):
    ...
    def execute(self):
        from ranger.ext.vcs import VcsError  # type: ignore
...
class linemode(default_linemode):
    ...
    def execute(self):
        ...
        if mode == "normal":
            from ranger.core.linemode import DEFAULT_LINEMODE  # type: ignore

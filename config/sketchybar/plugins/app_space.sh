...
create_label() {
  ...
  local CURRENT_APP
  CURRENT_APP=$(echo "$QUERY" | jq -r '.[] | select(.["has-focus"] == true) | .app')=$(echo "$QUERY" | jq -r '.[] | select(.["has-focus"] == true) | .app')
  local LABEL BADGE
  ...
  for APP in "${APPS[@]}"; do
    # add icon
    LABEL+=$("$CONFIG_DIR/plugins/app_icon.sh" "$APP")
    # set up badge
    BADGE="$(set_badge "$APP")"
...
mouse_clicked() {
  if [[ "$BUTTON" == "right" ]] || [[ "$MODIFIER" == "shift" ]]; then
    ...
    if SPACE_LABEL="$(osascript -e "return (text returned of (display dialog \"Rename space $SPACE_NAME to:\" default answer \"\" with title \"Space Renamer\" buttons {\"Cancel\", \"Rename\"} default button \"Rename\"))")"; then
      if [[ "$SPACE_LABEL" == "" ]]; then

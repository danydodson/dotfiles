...
local function get_filesystem_extra(file)
    ...
    -- Get device and filesystem info from mount first
    local mount_output = Command("mount"):stdout(Command.PIPED):output()
    if mount_output then
        for line in mount_output.stdout:gmatch("[^\n]+") do
            -- macOS mount output format: /dev/disk1s1 on / (apfs, ...)
            local device, path, fstype = line:match("([^%s]+) on ([^%s]+) %(([^,]+)")
            if path and file_url:match("^" .. path) then
    ...
    local child, err = Command("df"):arg({ "-h", file_url }):stdout(Command.PIPED):spawn()
    if child then
        -- Ignore header
        local _, event = child:read_line()
        if event ~= 0 then
            result.error = "df command failed"
            return result
        end
...
local function attributes(file)
	...
	-- macOS uses different attributes, we'll use ls -l@ instead
	local output = Command("ls"):arg({ "-l@", file_url }):stdout(Command.PIPED):output()
	if output then
		local attrs = ""
		for line in output.stdout:gmatch("[^\n]+") do
			if line:match("^%s+") then -- Extended attribute line
				attrs = attrs .. "@"
			end
		end
		return attrs

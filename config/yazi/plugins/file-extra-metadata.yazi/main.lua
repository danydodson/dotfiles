...
local function get_filesystem_extra(file)
	...
	local output, _ = Command("tail")
		:args({ "-n", "-1" })
		:stdin(Command("gdf"):args({ "-P", "-T", "-h", file_url }):stdout(Command.PIPED):spawn():take_stdout())
	...
	if output then
		...
		result.error = "tail, gdf are installed?"
	end
	return result
end
...
local function attributes(file)
	...
	local output, _ = Command("xattr"):args({ "-l", file_url }):stdout(Command.PIPED):output()

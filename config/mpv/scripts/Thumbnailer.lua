...
local function subprocess_result(sub_success, result, mpv_error, subprocess_name, start_time)
  ...
  if result then
    cmd_status, cmd_stdout, cmd_stderr, cmd_error, cmd_killed = result.status, result.stdout, result.stderr, result.error_string, result.killed_by_us
  end
  local cmd_status_success, cmd_status_string, cmd_err_success, cmd_err_string, success

  if cmd_status == 0 then
    cmd_status_success, cmd_status_string = true, 'ok'
  elseif is_empty(cmd_status) then
    cmd_status_success, cmd_status_string = true, '_'
  elseif cmd_status == 124 or cmd_status == 137 or cmd_status == 143 then -- timer: timed-out(124), killed(128+9), or terminated(128+15)
    ...
    cmd_status_success, cmd_status_string = false, ('%d'):format(cmd_status)
  end

  if is_empty(cmd_error) then
    cmd_err_success, cmd_err_string = true, '_'
  elseif cmd_error == 'init' then
    cmd_err_success, cmd_err_string = false, 'failed to initialize'
  elseif cmd_error == 'killed' then
    cmd_err_success, cmd_err_string = false, cmd_killed and 'killed by us' or 'killed, but not by us'
  else
    cmd_err_success, cmd_err_string = false, cmd_error
  end

  if is_empty(cmd_stdout) then
    cmd_stdout = '_'
  end
  if is_empty(cmd_stderr) then
    cmd_stderr = '_'
  end
  ...
  if success then
    msg.debug('Subprocess', subprocess_name, 'succeeded. | Status:', cmd_status_string, '| Time:', ('%ds'):format(os.difftime(os.time(), start_time)))
  else
    msg.error('Subprocess', subprocess_name, 'failed. | Status:', cmd_status_string, '| MPV Error:', mpv_error or 'n/a', '| Subprocess Error:', cmd_err_string, '| Stdout:', cmd_stdout, '| Stderr:', cmd_stderr)
  end
...
local function run_subprocess(command, name)
  ...
  local result, mpv_error = mp.command_native { name = 'subprocess', args = command, playback_only = false, capture_stdout = true, capture_stderr = true, env = { 'PATH=' .. os.getenv 'PATH' } }
...
local function run_subprocess_async(command, name)
  ...
  mp.command_native_async({ name = 'subprocess', args = command, playback_only = false, capture_stdout = true, capture_stderr = true, env = { 'PATH=' .. os.getenv 'PATH' } }, function(s, r, e)
    subprocess_result(s, r, e, subprocess_name, start_time)
  end)
  return nil
end
...
local function exec_exist(name, exec_path)
  local delim = ':'
  if OPERATING_SYSTEM == OS_WIN then
    delim, name = ';', name .. '.exe'
  end
  ...
  for path_dir in env_path:gmatch('[^' .. delim .. ']+') do
    if file_exists(join_paths(path_dir, name)) then
      return true
    end
  end
...
local function workers_reset()
  for _, timer in ipairs(workers_timers) do
    timer:kill()
  end
  workers_timers = {}
...
local function workers_start()
  timer_start = os.time()
  if state.cache_dir and state.cache_dir ~= '' then
    os.remove(join_paths(state.cache_dir, 'stop'))
  end
  for i, worker in ipairs(workers_indexed) do
    if i > state.max_workers then
      break
    ...
    table.insert(
      workers_timers,
      mp.add_timeout(user_opts.worker_delay * i ^ 0.8, function()
        mp.command_native { 'script-message-to', worker, message.worker.start }
      end)
    )
  end
...
local function workers_stop()
  ...
  if timer_total and timer_start then
    timer_total = timer_total + os.difftime(os.time(), timer_start)
  end
  timer_start = 0
end

local function workers_are_stopped()
  if not initialized or not workers_started then
    return true
  end
...
local function osc_reset()
  ...
  osc_visible = nil
  if osc_name then
    mp.command_native { 'script-message-to', osc_name, message.osc.reset }
  end
end
...
local osc_full_update_timer = mp.add_periodic_timer((4.00 * user_opts.update_time), function()
  osc_update(nil, nil, thumbnails)
end)
osc_full_update_timer:kill()
local osc_delta_update_timer = mp.add_periodic_timer((0.25 * user_opts.update_time), function()
  osc_delta_update()
end)
...
local count_existing = {
  [message.queued] = function()
    osc_stats.queued = osc_stats.queued - 1
  end,
  [message.processing] = function()
    osc_stats.processing = osc_stats.processing - 1
  end,
  [message.failed] = function()
    osc_stats.failed = osc_stats.failed - 1
  end,
  [message.ready] = function()
    osc_stats.ready = osc_stats.ready - 1
  end,
}
local count_new = {
  [message.queued] = function()
...
local function osc_update_count(time_string, status)
  ...
  if existing then
    count_existing[existing]()
  else
    osc_stats.total = osc_stats.total + 1
  end
  if status then
    count_new[status]()
  else
...
local function hash_string(filepath, filename)
  ...
  local command
  if exec_exist('shasum', user_opts.exec_path) then
    command = { user_opts.exec_path .. 'shasum', '-a', '256', filepath }
  elseif exec_exist('gsha256sum', user_opts.exec_path) then
    command = { user_opts.exec_path .. 'gsha256sum', filepath }
  elseif exec_exist('sha256sum', user_opts.exec_path) then
    command = { user_opts.exec_path .. 'sha256sum', filepath }
  end
  if not command then
    return filename
  end -- checksum command unavailable
...
local function create_ouput_dir(filepath, filename, dimension, rotate)
  ...
  if not create_dir(fullpath) then
    return { basepath = nil, fullpath = nil }
  end
...
local function calculate_timing(is_remote)
  ...
  if duration == 0 then
    return { duration = 0, delta = huge, high_bitrate = false }
  end
...
local function calculate_geometry(scale)
  ...
  if not video_params or is_empty(video_params.dw, video_params.dh) or dimension <= 0 then
    return geometry
  end
  ...
  if not video_params.rotate then
    return geometry
  end
...
local function calculate_worker_timeout(width, height, is_remote, is_high_bitrate)
  if user_opts.worker_timeout == 0 then
    return 0
  end
  local worker_timeout = ((width * height) / 921600) * user_opts.worker_timeout
  if is_remote then
    worker_timeout = worker_timeout * 2
  end
  if is_high_bitrate then
    worker_timeout = worker_timeout * 2
  end
...
local function state_init()
  ...
  if is_empty(worker_script_path) then
    worker_script_path = user_opts.worker_script_path
  end
  create_workers()
  initialized = true
end
...
local function is_thumbnailable()
  ...
  if not (state and stop_conditions) then
    return false
  end
  for key, value in pairs(state) do
    if key == 'rotate' and value then
      goto continue
    end
    if key == 'worker_buffer' and value then
      goto continue
    end
    if key == 'osc_buffer' and value then
      goto continue
    end
    if key == 'worker_timeout' and value then
      goto continue
    end
...
local function delete_cache_dir()
  if auto_delete == nil then
    auto_delete = user_opts.auto_delete
  end
...
local function reset_all(keep_saved, keep_osc_data)
  ...
  if not keep_saved or not saved_state then
    saved_state_init()
  end
  if not keep_osc_data then
    osc_reset()
  else
    osc_reset_stats()
  end
...
local function run_generation(paused)
  if not initialized or not is_thumbnailable() then
    return
  end
...
local function start(paused)
  if not initialized then
    mp.add_timeout(user_opts.start_delay, function()
      state_init()
      start(paused)
    end)
  end
...
local function osc_set_visibility(is_visible)
  if is_visible and not initialized then
    start(true)
  end
  if osc_name then
    osc_update(nil, osc_set_options(is_visible), nil)
  end
end
...
mp.register_script_message(message.toggle_gen, function()
  if workers_are_stopped() then
    start()
  else
    stop()
  end
...
mp.register_script_message(message.manual_show, function()
  osc_set_visibility(true)
end)

-- Binding - Manual Hide OSC
mp.register_script_message(message.manual_hide, function()
  osc_set_visibility(false)
end)

-- Binding - Toggle Visibility
mp.register_script_message(message.toggle_osc, function()
  osc_set_visibility(not osc_visible)
end)

-- Binding - Double Frequency
mp.register_script_message(message.double, function()
  if not initialized or not saved_state or not saved_state.delta_factor then
    return
  end
...
mp.register_script_message(message.shrink, function()
  if initialized and saved_state and saved_state.size_factor then
    resize(max(0.2, saved_state.size_factor - 0.2))
  end
end)
...
mp.register_script_message(message.enlarge, function()
  if initialized and saved_state and saved_state.size_factor then
    resize(min(2.0, saved_state.size_factor + 0.2))
  end
end)
...
mp.observe_property('video-params', 'native', function(_, video_params)
  if not video_params or is_empty(video_params.dw, video_params.dh) then
    return
  end
...
mp.observe_property('fullscreen', 'native', function(_, fullscreen)
  if (fullscreen == nil) or (not osc_opts or osc_opts.scalewindowed == osc_opts.scalefullscreen) then
    return
  end
...
mp.register_script_message(message.worker.progress, function(json)
  ...
  if new_progress.input_filename ~= state.input_filename then
    return
  end

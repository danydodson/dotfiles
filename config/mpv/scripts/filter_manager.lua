...
local cmd = {
  enable = function(filter)
    mp.command_native { type_map[filter.filter_type], 'add', ('@%s:%s'):format(filter.name, filter.filters[filter.current_index]) }
  end,
  ...
  remove = function(filter)
    mp.command_native { type_map[filter.filter_type], 'del', ('@%s'):format(filter.name) }
  end,
  clear = function()
    mp.set_property_native(type_map['audio'], {})
    mp.set_property_native(type_map['video'], {})
  end,
}
...
local function toggle_filter(filter, no_osd)
  ...
  if filter.current_index == 0 then
...
local function filter_status(filter, no_osd)
  mp.command_native_async({ 'script-message', filter.name .. '-state', filter.enabled and filter.filters[filter.current_index] or '' }, function()
    show_status(filter, no_osd)
  end)
end

local function register_filter(filter)
  if filter.default_on_load == nil then
    filter.default_on_load = defaults.default_on_load
  end
  if filter.reset_on_load == nil then
    filter.reset_on_load = defaults.reset_on_load
  end
  ...
  mp.register_script_message(filter.name .. '-cycle+', function(no_osd)
    cycle_filter_up(filter, no_osd == 'yes')
  end)
  mp.register_script_message(filter.name .. '-cycle-', function(no_osd)
    cycle_filter_dn(filter, no_osd == 'yes')
  end)
  mp.register_script_message(filter.name .. '-toggle', function(no_osd)
    toggle_filter(filter, no_osd == 'yes')
  end)
  mp.register_script_message(filter.name .. '-enable', function(no_osd)
    enable_filter(filter, no_osd == 'yes')
  end)
  mp.register_script_message(filter.name .. '-disable', function(no_osd)
    disable_filter(filter, no_osd == 'yes')
  end)
  mp.register_script_message(filter.name .. '-status', function(no_osd)
    filter_status(filter, no_osd == 'yes')
  end)

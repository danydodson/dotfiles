...
local function show_status(profile, eq)
  ...
  local eq_string = eq and ('%s%+4.2d'):format(lables[eq], profile[eq]) or ('Brightness%+4.2d | Contrast%+4.2d | Gamma%+4.2d | Hue%+4.2d | Saturation%+4.2d'):format(profile.brightness, profile.contrast, profile.gamma, profile.hue, profile.saturation)
...
for _, eq in pairs(eqs) do
  mp.register_script_message('Eq-' .. eq, function(amount)
    on_eq_change(eq, amount)
  end)
  mp.register_script_message('Eq-' .. eq .. '-set', function(value)
    on_eq_set(eq, value)
  end)
end

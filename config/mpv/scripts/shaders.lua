...
local function is_initialized()
  return (
    (props['dwidth'] > 0)
    and (props['dheight'] > 0)
    and (props['display-width'] > 0)
    and (props['display-height'] > 0)
    and (props['osd-dimensions/w'] > 0)
    and (props['osd-dimensions/h'] > 0)
    and (props['osd-dimensions/mt'] >= 0)
    and (props['osd-dimensions/mb'] >= 0)
    and (props['osd-dimensions/ml'] >= 0)
    and (props['osd-dimensions/mr'] >= 0)
    and (props['container-fps'] > 0)
    and (props['video-params/rotate'] >= 0)
    and (type(props['video-params/colormatrix']) == 'string')
    and (props['video-params/colormatrix'] ~= '')
    and true
  )
end
...
local function is_high_fps()
  return props['container-fps'] >= opts.hifps_threshold
end
local function is_low_fps()
  return props['container-fps'] <= opts.lowfps_threshold
end
local function is_hdr()
  return props['video-params/colormatrix']:find 'bt.2020' ~= nil
end
local function is_rgb()
  return props['video-params/colormatrix']:find 'rgb' ~= nil
end
...
local function minmax(v, min, max)
  return math.min(math.max(v, min), max)
end
local function minmax_scale(min, max)
  return math.floor(minmax(get_scale(), min, max) + 0.11)
end
...
local g13 = { blur = g13 .. 'gaussianBlur.glsl', }
...
local function set_default_index()
  if not opts.auto_switch then
    return
  end
  local path = mp.get_property_native('path', ''):lower()
  current_index = opts.default_index
  if opts.preset_4_enabled and path:find(opts.preset_4_path) ~= nil then
    current_index = opts.preset_4_index
  end
  if opts.preset_3_enabled and path:find(opts.preset_3_path) ~= nil then
    current_index = opts.preset_3_index
  end
  if opts.preset_2_enabled and path:find(opts.preset_2_path) ~= nil then
    current_index = opts.preset_2_index
  end
  if opts.preset_1_enabled and path:find(opts.preset_1_path) ~= nil then
    current_index = opts.preset_1_index
  end
  if opts.preset_rgb_enabled and is_rgb() then
    current_index = opts.preset_rgb_index
  end
  if opts.preset_lowfps_enabled and is_low_fps() then
    current_index = opts.preset_lowfps_index
  end
  if opts.preset_hifps_enabled and is_high_fps() then
    current_index = opts.preset_hifps_index
  end
  msg.debug('Default Index:', current_index)
end

local timer = mp.add_timeout(opts.set_timer, function()
  set_shaders(true)
end)
timer:kill()
local firstrun = true
...
mp.register_script_message('Shaders-cycle+', function(no_osd)
  cycle_set_up(no_osd == 'yes')
end)
mp.register_script_message('Shaders-cycle-', function(no_osd)
  cycle_set_dn(no_osd == 'yes')
end)
mp.register_script_message('Shaders-toggle', function(no_osd)
  toggle_set(no_osd == 'yes')
end)
mp.register_script_message('Shaders-enable', function(no_osd)
  enable_set(no_osd == 'yes')
end)
mp.register_script_message('Shaders-disable', function(no_osd)
  disable_set(no_osd == 'yes')
end)
mp.register_script_message('Shaders-show', function(no_osd)
  show_set(no_osd == 'yes')
end)

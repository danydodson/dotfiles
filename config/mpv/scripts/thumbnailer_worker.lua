...
local function subprocess_result(sub_success, result, mpv_error, subprocess_name, start_time)
  ...
  if success then
    ...
    msg.error('Subprocess', subprocess_name, 'failed. | Status:', cmd_status_string, '| MPV Error:', mpv_error or 'n/a', '| Subprocess Error:', cmd_err_string, '| Stdout:', cmd_stdout, '| Stderr:', cmd_stderr, '| Time:', ('%ds'):format(os.difftime(os.time(), start_time)))
  end
...
local transpose = { [-360] = '', [-270] = ',transpose=1', [-180] = ',transpose=2,transpose=2', [-90] = ',transpose=2', [0] = '', [90] = ',transpose=1', [180] = ',transpose=1,transpose=1', [270] = ',transpose=2', [360] = '' }
...
local function create_ffmpeg_command(time, output, force_accurate_seek)
  ...
  if args then
    ...
    if OPERATING_SYSTEM ~= OS_WIN and worker_options.worker_timeout > 0 then
      add_args(args, '-timelimit', ceil(worker_options.worker_timeout))
    end
...
local function report_progress(index, new_status)
  ...
  if index ~= nil and thumbnail_map_buffer[index_string] == nil then
    thumbnail_map_buffer_size = thumbnail_map_buffer_size + 1
  end
...
local function create_thumbnail(time, fullpath)
  return (run_subprocess(worker_extra.create_command(time, fullpath, false)) and check_existing(fullpath)) or (run_subprocess(worker_extra.create_command(time, fullpath, true)) and check_existing(fullpath))
end
...
local function process_queue()
  if not work_queue then
    return
  end
  for _ = 1, #work_queue do
    if stop_file_exist() then
      report_progress()
      break
    end
    process_thumbnail()
  ...
  report_progress()
  if #work_queue == 0 then
    mp.command_native { 'script-message', message.worker.finish, format_json(worker_stats) }
  end
end
...
mp.register_script_message(message.worker.queue, function(json)
  ...
  if new_data.worker_options then
    worker_options = new_data.worker_options
  end
  if new_data.start_time_index then
    worker_data.start_time_index = new_data.start_time_index
  end
  if not worker_extra.filesize then
    worker_extra.filesize = (state.width * state.height * 4)
  end
  create_queue()
end)

...
local user_opts = {
  ...
  title = '${!playlist-count==1:[${playlist-pos-1}/${playlist-count}] }${media-title}', -- to be shown as OSC title
  tooltipborder = 1, -- border of tooltip in bottom/topbar
  timetotal = false, -- display total time instead of remaining time?
  remaining_playtime = true, -- display the remaining time in playtime or video-time mode playtime takes speed into account, whereas video-time doesn't
  ...
  font_mono = 'FiraMono Nerd Font Mono',
...
local function update_margins()
  ...
  if not state.osc_visible or get_hidetimeout() >= 0 or (state.fullscreen and not user_opts.showfullscreen) or (not state.fullscreen and not user_opts.showwindowed) then
...
local function update_tn_osc_params(seek_y)
  ...
  if tn_osc.scale.x_last ~= tn_osc.scale.x or tn_osc.scale.y_last ~= tn_osc.scale.y or tn_osc.w_last ~= tn_state.width or tn_osc.h_last ~= tn_state.height or tn_osc.osd.w_last ~= tn_osc.osd.w or tn_osc.osd.h_last ~= tn_osc.osd.h then
...
local function show_thumbnail(seek_percent)
  ...
  if thumbnail.seek_index_last ~= seek_index or thumbnail.ready_last ~= ready or thumbnail.total_expected_last ~= tn_osc_stats.total_expected then
...
local function display_tn_osc(seek_y, seek_percent, ass)
  if not (seek_y and seek_percent and ass and tn_state and tn_osc_stats and tn_osc_options and tn_state.width and tn_state.height and tn_state.duration and tn_state.cache_dir) or not tn_osc_options.visible then
    hide_thumbnail()
    return
  end
...
local function prepare_elements()
  ...
  for _, element in pairs(elements) do
    ...
    elseif element.type == 'slider' then
      ...
      -- the "hole"
      ass_draw_rr_h_ccw(static_ass, slider_lo.border, slider_lo.border, elem_geo.w - slider_lo.border, elem_geo.h - slider_lo.border, r2, slider_lo.stype == 'diamond')
        ...
        for _, marker in pairs(markers) do
          if marker > element.slider.min.value and marker < element.slider.max.value then
            ...
            if slider_lo.gap > 1 then -- draw triangles
              ...
              --bottom
              if slider_lo.nibbles_bottom then
                static_ass:rect_cw(s - 1, elem_geo.h - slider_lo.border - slider_lo.gap, s + 1, elem_geo.h - slider_lo.border)
              end
            end
          end
        end
    ...
    if element.type == 'slider' then
      ...
      if pos then
        ...
        if slider_lo.rtype == 'slider' then
          ass_draw_rr_h_cw(elem_ass, foH - innerH / 6, foH - innerH / 6, xp, foH + innerH / 6, innerH / 6, slider_lo.stype == 'diamond', 0)
          ass_draw_rr_h_cw(elem_ass, xp, foH - innerH / 15, elem_geo.w - foH + innerH / 15, foH + innerH / 15, 0, slider_lo.stype == 'diamond', innerH / 15)
          for _, range in pairs(seekRanges or {}) do
            local pstart = get_slider_ele_pos_for(element, range['start'])
            local pend = get_slider_ele_pos_for(element, range['end'])
            ass_draw_rr_h_ccw(elem_ass, pstart, foH - innerH / 21, pend, foH + innerH / 21, innerH / 21, slider_lo.stype == 'diamond')
          end
        end
      end
      ...
      if seekRanges then
        ...
        for _, range in pairs(seekRanges) do
          ...
          if slider_lo.rtype == 'slider' then
            ass_draw_rr_h_cw(elem_ass, pstart, foH - innerH / 21, pend, foH + innerH / 21, innerH / 21, slider_lo.stype == 'diamond')
          elseif slider_lo.rtype == 'line' then
            if slider_lo.stype == 'bar' then
              elem_ass:rect_cw(pstart, elem_geo.h - foV - seekRangeLineHeight, pend, elem_geo.h - foV)
            else
              ass_draw_rr_h_cw(elem_ass, pstart - innerH / 8, foH - innerH / 8, pend + innerH / 8, foH + innerH / 8, innerH / 8, slider_lo.stype == 'diamond')
            end
          elseif slider_lo.rtype == 'bar' then
            if slider_lo.stype ~= 'bar' then
              ass_draw_rr_h_cw(elem_ass, pstart - innerH / 2, foV, pend + innerH / 2, foV + innerH, innerH / 2, slider_lo.stype == 'diamond')
          ...
          elseif slider_lo.rtype == 'inverted' then
            if slider_lo.stype ~= 'bar' then
              ass_draw_rr_h_ccw(elem_ass, pstart, (elem_geo.h / 2) - 1, pend, (elem_geo.h / 2) + 1, 1, slider_lo.stype == 'diamond')
            else
...
local function window_controls(topbar)
  ...
  lo.style = string.format('%s{\\clip(%f,%f,%f,%f)}', osc_styles.wcTitle, titlebox_left + left_pad, wc_geo.y - wc_geo.h, titlebox_right - right_pad, wc_geo.y + wc_geo.h)
...
layouts['box'] = function()
  ...
  if user_opts.valign > 0 then
    ...
    -- deadzone below OSC
    sh_area_y0 = 0
    sh_area_y1 = (posY + (osc_geo.h / 2)) + get_align(1 - (2 * user_opts.deadzonesize), osc_param.playresy - (posY + (osc_geo.h / 2)), 0, 0)
  end
...
local function osc_init()
  ...
  ne.content = function()
    ...
    return mp.get_property 'play-direction' == 'forward' and icons.play or icons.play_backward
  end
  ...
  ne.eventresponder['mbtn_mid'] = function()
  ...
  ne.content = function()
    return icons.audio .. osc_styles.smallButtonsLlabel .. ' ' .. mp.get_property_number('aid', '-') .. '/' .. audio_track_count
  end
  ...
  ne.content = function()
    return icons.subtitle .. osc_styles.smallButtonsLlabel .. ' ' .. mp.get_property_number('sid', '-') .. '/' .. sub_track_count
  end
  ...
  ne.enabled = mp.get_property 'percent-pos' ~= nil and user_opts.layout ~= 'slimbottombar' and user_opts.layout ~= 'slimtopbar'
  ...
  if user_opts.scrollcontrols then
    ne.eventresponder['wheel_up_press'] = function()
      mp.commandv('osd-auto', 'seek', -1.00)
    end
    ne.eventresponder['wheel_down_press'] = function()
      mp.commandv('osd-auto', 'seek', 0.01)
    end
  end

  ne.eventresponder['mbtn_right_down'] = function(element)
    mp.commandv('script-message', 'Thumbnailer-toggle-osc')
  end
  ne.eventresponder['mbtn_right_dbl_press'] = function(element)
    mp.commandv('script-message', 'Thumbnailer-double')
  end
...
local function process_event(source, what)
  ...
  elseif source == 'mouse_move' then
    ...
    if user_opts.minmousemove == 0 or ((state.last_mouseX ~= nil and state.last_mouseY ~= nil) and ((math.abs(mouseX - state.last_mouseX) >= user_opts.minmousemove) or (math.abs(mouseY - state.last_mouseY) >= user_opts.minmousemove))) then
      show_osc()
    end
...
mp.register_script_message('osc-message', function(message, dur)
  if not state.osc_message_warned then
    mp.msg.warn('osc-message is deprecated and may be removed in the future.', 'Use the show-text command instead.')
...
mp.register_script_message('osc-chapterlist', function(dur)
  if not state.osc_chapterlist_warned then
    mp.msg.warn('osc-chapterlist is deprecated and may be removed in the future.', 'Use show-text ${chapter-list} instead.')
...
mp.register_script_message('osc-playlist', function(dur)
  if not state.osc_playlist_warned then
    mp.msg.warn('osc-playlist is deprecated and may be removed in the future.', 'Use show-text ${playlist} instead.')
...
mp.register_script_message('osc-tracklist', function(dur)
  if not state.osc_tracklist_warned then
    mp.msg.warn('osc-tracklist is deprecated and may be removed in the future.', 'Use show-text ${track-list} instead.')
...
mp.set_key_bindings({
  ...
  { 'shift+mbtn_left_dbl', 'ignore' },
  {
    'mbtn_right_dbl',
    function()
      process_event('mbtn_right_dbl', 'press')
    end,
  },
}, 'input', 'force')
mp.enable_key_bindings 'input'
...
local function validate_user_opts()
  ...
  if user_opts.seekrangestyle ~= 'bar' and user_opts.seekrangestyle ~= 'line' and user_opts.seekrangestyle ~= 'slider' and user_opts.seekrangestyle ~= 'inverted' and user_opts.seekrangestyle ~= 'none' then
  ...
  local colors = {
    ...
    user_opts.top_buttons_color,
    user_opts.small_buttonsL_color,
    user_opts.small_buttonsR_color,
    user_opts.buttons_color,
    user_opts.title_color,
    user_opts.timecode_color,
    user_opts.time_pos_color,

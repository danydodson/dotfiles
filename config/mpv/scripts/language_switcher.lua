...
local function process_tracks(subtracks, lang_priority)
  if not (subtracks and lang_priority) then
    return
  end
...
local function add_undefined_tracks(subtracks, processed_subtracks, language_first_index)
  if not (subtracks and processed_subtracks and language_first_index) then
    return
  end
  local title, is_sign_song
  for _, subtrack in ipairs(subtracks) do
...
local saved
local function reset_saved(path)
  saved = { audio = { current_index = nil, active = false }, sub = { current_index = nil, active = false }, last_path = path }
end
reset_saved ''

local function set_track(track_type, subtracks, no_osd)
  if not (track_type and subtracks and subtracks.current_index) then
    return
  end
  local subtrack = subtracks[subtracks.current_index]
  if not (subtrack and subtrack.lang and subtrack.id) then
    return
  end
...
local set_track_active = {
  ...
  audio = function(active, no_osd)
    ...
    mp.command_native_async({ 'set', 'mute', active and 'no' or 'yes' }, function()
      if not no_osd then
        mp.osd_message((active and '■' or '□') .. ' Audio')
      end
    end)
  end,
  sub = function(active, no_osd)
    saved.sub.active = active
    mp.command_native_async({ 'set', 'sub-visibility', active and 'yes' or 'no' }, function()
      if not no_osd then
        mp.osd_message((active and '■' or '□') .. ' Subtitle')
      end
    end)
  end,
}
...
mp.observe_property('track-list', 'native', function(_, track_list)
  ...
  if not track_list or #track_list == 0 or #track_list == track_count or track_count == -1 then
    return
  end
...
for track_type, _ in pairs(track_types) do
  mp.register_script_message(track_type .. '-track+', function()
    ...
    if not tracks[track_type].current_index then
      return
    end

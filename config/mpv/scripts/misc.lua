...
mp.register_script_message('Add', function(property, value)
  change_prop('add', property, value)
end)
mp.register_script_message('Set', function(property, value)
  change_prop('set', property, value)
end)
...
-- Show Play/Pause
local display = mp.get_property_osd('osd-ass-cc/0', '') .. '{\\1a&H20&\\3a&H20&\\bord1\\blur0.5\\fs20\\fnmpv-osd-symbols}%s' .. mp.get_property_osd('osd-ass-cc/1', '')
...
mp.observe_property('ontop', 'native', function(_, ontop)
  if ontop == nil or ontop == last_ontop then
    return
  end
  last_ontop = ontop
...
mp.observe_property('interpolation', 'native', function(_, interpolation)
  if interpolation == nil or interpolation == last_interpolation then
    return
  end
...
mp.observe_property('deband', 'native', function(_, deband)
  if deband == nil or deband == last_deband then
    return
  end
  last_deband = deband

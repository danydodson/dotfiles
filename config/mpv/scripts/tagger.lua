...
local function run_tag(cmd)
  if not file_exists(cmd[#cmd]) then
    return
  end
...
local function add_tag(path, tag)
  return run_tag { user_opts.exec_path .. 'tag', '-a', tag, path }
end
local function del_tag(path, tag)
  return run_tag { user_opts.exec_path .. 'tag', '-r', tag, path }
end
local function read_tag(path)
  return run_tag { user_opts.exec_path .. 'tag', '-l', '-N', '-g', path }
end
...
mp.register_script_message('Tag-add', function(tag)
  ...
  if current_tags and not current_tags[tag] then
    add_tag(path, tag)
  end
...
mp.register_script_message('Tag-del', function(tag)
  ...
  local current_tags = read_tag(path)
  if tag == '\\*' or (current_tags and current_tags[tag]) then
    del_tag(path, tag)
  end
  show_tags(read_tag(path))
end)
...
mp.register_script_message('Tag-toggle', function(tag)
  ...
  local current_tags = read_tag(path)
  if current_tags and current_tags[tag] then
    del_tag(path, tag)
  else
    add_tag(path, tag)
  end

...
local function text_style()
  ...
  if o.custom_header and o.custom_header ~= '' then
    ...
    local style = '{\\r\\an4\\fs' .. font_size .. '\\bord' .. border_size
...
local function generate_graph(values, i, len, v_max, v_avg, scale, x_tics)
  ...
  local bg_box = format('{\\bord%f}{\\3c&H%s&}{\\1c&H%s&}m 0 %f l %f %f %f 0 0 0', plot_bg_border_width, o.plot_bg_border_color, o.plot_bg_color, y_max, x_max, y_max, x_max)
  return format('%s{\\rDefault}{\\pbo%f}{\\shad0}{\\alpha&H00}{\\p1}%s{\\p0}' .. '{\\bord0}{\\1c&H%s}{\\p1}%s{\\p0}%s', o.prefix_sep, y_offset, bg_box, o.plot_color, table.concat(s), text_style())
end
...
local function append(s, str, attr)
  ...
  s[index] = s[index] or ''
  s[index] = s[index] .. format('%s%s%s%s%s%s', attr.nl, attr.indent, attr.prefix, attr.prefix_sep, no_ASS(str), attr.suffix)
  return true
end
...
local function append_perfdata(header, s, dedicated_page)
  ...
  h[#h + 1] = format('%s%s%s%s%s%s%s%s', dedicated_page and '' or o.nl, dedicated_page and '' or o.indent, bold 'Frame Timings:', o.prefix_sep, font_small, '(last | average | peak ms)', font_normal, dedicated_page and scroll_hint() or '')
  ...
  for _, frame in ipairs(sorted_keys(vo_p)) do -- ensure fixed display order
    ...
    if dedicated_page then
      ...
      for _, pass in ipairs(data) do
        ...
        desc = desc:find 'user shader: ' == nil and '{\\b400}' .. desc .. '{\\b0}' or '{\\b600}' .. desc:gsub('user shader: ', '') .. '{\\b0}'

        s[#s + 1] = format(f, o.nl, o.indent, o.indent, font_mono, pp(pass['last']), pp(pass['avg']), pp(pass['peak']), o.prefix_sep .. indent, p(pass['last'], last_s[frame]), font, o.prefix_sep, o.prefix_sep, desc)

        if o.plot_perfdata and o.use_ass then
          -- use the same line that was already started for this iteration
          s[#s] = s[#s] .. generate_graph(pass['samples'], pass['count'], pass['count'], pass['peak'], pass['avg'], 0.9, 0.25)
        end
      end

      -- Print sum of timing values as "Total"
      s[#s + 1] = format(f, o.nl, o.indent, o.indent, font_mono, pp(last_s[frame]), pp(avg_s[frame]), pp(peak_s[frame]), o.prefix_sep, bold 'Total', font, '', '', '')
    else
      -- for the simplified view, we just print the sum of each pass
      s[#s + 1] = format(f, o.nl, o.indent, o.indent, font_mono, pp(last_s[frame]), pp(avg_s[frame]), pp(peak_s[frame]), '', '', font, o.prefix_sep, o.prefix_sep, frame:gsub('^%l', string.upper))
    end
  end
end
...
local function get_kbinfo_lines()
  ...
  for _, bind in pairs(bindings) do
    if bind.priority >= 0 and (not active[bind.key] or (active[bind.key].is_weak and not bind.is_weak) or (bind.is_weak == active[bind.key].is_weak and bind.priority > active[bind.key].priority)) and not bind.cmd:find('script-binding stats/__forced_', 1, true) and bind.section ~= 'input_forced_console' and (searched_text == nil or (bind.key .. bind.cmd .. (bind.comment or '')):lower():find(searched_text, 1, true)) then
  ...
  local spre = term and kspaces .. '   ' or format('{\\q2\\fn%s}%s   {\\fn%s}{\\fs%d\\u1}', o.font_mono, kspaces, o.font, 1.3 * font_size)
...
local function append_display_sync(s)
  ...
  if vspeed then
    append_property(s, 'audio-speed-correction', { prefix = '/', nl = '', indent = ' ', prefix_sep = ' ', no_prefix_markup = true })
  else
    append_property(s, 'audio-speed-correction', { prefix = 'DS:' .. o.prefix_sep .. ' - / ', prefix_sep = '' })
  end

  append_property(s, 'mistimed-frame-count', { prefix = 'Mistimed:', nl = '', indent = o.prefix_sep .. o.prefix_sep })
  append_property(s, 'vo-delayed-frame-count', { prefix = 'Delayed:', nl = '', indent = o.prefix_sep .. o.prefix_sep })
  ...
  if not display_timer.oneshot and (o.plot_vsync_ratio or o.plot_vsync_jitter) and o.use_ass then
    ...
    append_property(s, 'vsync-jitter', { prefix = 'VSync Jitter:', nl = vr and '' or o.nl, indent = vr and o.prefix_sep .. o.prefix_sep })
  end
end
...
local function add_file(s, print_cache, print_tags)
  ...
  if print_tags then
    ...
    for _, tag in ipairs(tags) do
      ...
      if tag ~= 'Title' and tags_displayed < o.file_tag_max_count and value and value:len() < o.file_tag_max_length then
  ...
  if ed_cond then
    append_property(s, 'edition-list/' .. tostring(edition) .. '/title', { prefix = 'Edition:' })
    append_property(s, 'edition-list/count', { prefix = '(' .. tostring(edition + 1) .. '/', suffix = ')', nl = '', indent = ' ', prefix_sep = ' ', no_prefix_markup = true })
  end

  local ch_index = mp.get_property_number 'chapter'
  if ch_index and ch_index >= 0 then
    append_property(s, 'chapter-list/' .. tostring(ch_index) .. '/title', { prefix = 'Chapter:', nl = ed_cond and '' or o.nl })
    append_property(s, 'chapter-list/count', { prefix = '(' .. tostring(ch_index + 1) .. ' /', suffix = ')', nl = '', indent = ' ', prefix_sep = ' ', no_prefix_markup = true })
  end

  local fs = append_property(s, 'file-size', { prefix = 'Size:' })
  append_property(s, 'file-format', { prefix = 'Format/Protocol:', nl = fs and '' or o.nl, indent = fs and o.prefix_sep .. o.prefix_sep })
  ...
  if demuxer_cache + demuxer_secs > 0 then
    ...
    append(s, format('%.1f', demuxer_secs), { prefix = '(', suffix = ' sec)', nl = '', no_prefix_markup = true, prefix_sep = '', indent = o.prefix_sep })
  end
end
...
local function crop_equal(r, ro)
  return r['crop-x'] == ro['crop-x'] and r['crop-y'] == ro['crop-y'] and r['crop-w'] == ro['crop-w'] and r['crop-h'] == ro['crop-h']
end
...
local function append_resolution(s, r, prefix, w_prop, h_prop, video_res)
  ...
  if append(s, r[w_prop], { prefix = prefix }) then
    append(s, r[h_prop], { prefix = 'x', nl = '', indent = ' ', prefix_sep = ' ', no_prefix_markup = true })
    if r['aspect'] ~= nil and not video_res then
      append(s, format('%.2f:1', r['aspect']), { prefix = '', nl = '', indent = '', no_prefix_markup = true })
      append(s, r['aspect-name'], { prefix = '(', suffix = ')', nl = '', indent = ' ', prefix_sep = '', no_prefix_markup = true })
    end
    if r['sar'] ~= nil and video_res then
      append(s, format('%.2f:1', r['sar']), { prefix = '', nl = '', indent = '', no_prefix_markup = true })
      append(s, r['sar-name'], { prefix = '(', suffix = ')', nl = '', indent = ' ', prefix_sep = '', no_prefix_markup = true })
    end
    if r['s'] then
      append(s, format('%.2f', r['s']), { prefix = '(', suffix = 'x)', nl = '', indent = o.prefix_sep, prefix_sep = '', no_prefix_markup = true })
    end
    ...
    if r['crop-w'] and (not video_res or not crop_noop(r[w_prop], r[h_prop], r)) then
      append(s, format('[x: %d, y: %d, w: %d, h: %d]', r['crop-x'], r['crop-y'], r['crop-w'], r['crop-h']), { prefix = '', nl = '', indent = '', no_prefix_markup = true })
    end
  end
end
...
local function append_hdr(s, hdr, video_out)
  ...
  if should_show(hdr['max-cll']) or should_show(hdr['max-luma']) then
    ...
    if hdr['min-luma'] and should_show(hdr['max-luma']) then
      ...
      append(s, format('%.2g / %.0f', hdr['min-luma'], hdr['max-luma']), { prefix = display_prefix, suffix = ' cd/m²', nl = '', indent = indent })
      indent = o.prefix_sep .. o.prefix_sep
    end
    if should_show(hdr['max-cll']) then
      append(s, hdr['max-cll'], { prefix = 'MaxCLL:', suffix = ' cd/m²', nl = '', indent = indent })
      indent = o.prefix_sep .. o.prefix_sep
    end
    if hdr['max-fall'] and hdr['max-fall'] > 0 then
      append(s, hdr['max-fall'], { prefix = 'MaxFALL:', suffix = ' cd/m²', nl = '', indent = indent })
    end
  end
  ...
  if hdr['scene-max-r'] or hdr['scene-max-g'] or hdr['scene-max-b'] or hdr['scene-avg'] then
    append(s, '', { prefix = 'HDR10+:' })
    append(s, format('%.1f / %.1f / %.1f', hdr['scene-max-r'] or 0, hdr['scene-max-g'] or 0, hdr['scene-max-b'] or 0), { prefix = 'MaxRGB:', suffix = ' cd/m²', nl = '', indent = '' })
    append(s, format('%.1f', hdr['scene-avg'] or 0), { prefix = 'Avg:', suffix = ' cd/m²', nl = '', indent = indent })
  end

  if hdr['max-pq-y'] and hdr['avg-pq-y'] then
    append(s, '', { prefix = 'PQ(Y):' })
    append(s, format('%.2f cd/m² (%.2f%% PQ)', pq_eotf(hdr['max-pq-y']), hdr['max-pq-y'] * 100), { prefix = 'Max:', nl = '', indent = '' })
    append(s, format('%.2f cd/m² (%.2f%% PQ)', pq_eotf(hdr['avg-pq-y']), hdr['avg-pq-y'] * 100), { prefix = 'Avg:', nl = '', indent = indent })
  end
end
...
local function add_video_out(s)
  ...
  append(s, get_property_cached 'display-names', { prefix_sep = '', prefix = '(', suffix = ')', no_prefix_markup = true, nl = '', indent = ' ' })
  append(s, mp.get_property_native 'current-gpu-context', { prefix = 'Context:', nl = '', indent = o.prefix_sep .. o.prefix_sep })
  append_property(s, 'avsync', { prefix = 'A-V:' })
  append_fps(s, 'display-fps', 'estimated-display-fps')
  if append_property(s, 'decoder-frame-drop-count', { prefix = 'Dropped Frames:', suffix = ' (decoder)' }) then
  ...
  if not mp.get_property_native 'fullscreen' then
    ...
    scale = ('%.3f'):format(math.sqrt((scaled_width * scaled_height) / (r['w'] * r['h'])))
  end
...
local function add_video(s)
  ...
  if track then
    ...
    append(s, track['codec-profile'], { prefix = '[', nl = '', indent = ' ', prefix_sep = '', no_prefix_markup = true, suffix = ']' })
    if track['codec'] ~= track['decoder'] then
      append(s, track['decoder'], { prefix = '[', nl = '', indent = ' ', prefix_sep = '', no_prefix_markup = true, suffix = ']' })
    end
    append(s, get_property_cached 'hwdec-current', { prefix = 'HW:', nl = '', indent = o.prefix_sep .. o.prefix_sep, no_prefix_markup = false, suffix = '' }, { no = true, [''] = true })
  end
  local has_prefix = false
  if o.show_frame_info then
    if append_property(s, 'estimated-frame-number', { prefix = 'Frame:' }) then
      append_property(s, 'estimated-frame-count', { indent = ' / ', nl = '', prefix_sep = '' })
      has_prefix = true
    end
    local frame_info = mp.get_property_native 'video-frame-info'
    if frame_info and frame_info['picture-type'] then
      local attrs = has_prefix and { prefix = '(', suffix = ')', indent = ' ', nl = '', prefix_sep = '', no_prefix_markup = true } or { prefix = 'Picture Type:' }
      append(s, frame_info['picture-type'], attrs)
      has_prefix = true
    end
    if frame_info and frame_info['interlaced'] then
      local attrs = has_prefix and { indent = ' ', nl = '', prefix_sep = '' } or { prefix = 'Picture Type:' }
    ...
    for prop, name in pairs(timecodes) do
      if frame_info and frame_info[prop] then
        local attrs = has_prefix and { prefix = name .. ' Timecode:', indent = o.prefix_sep .. o.prefix_sep, nl = '' } or { prefix = name .. ' Timecode:' }
...
local function add_audio(s)
  ...
  append_property(s, 'current-ao', { prefix = 'AO:', nl = '', indent = o.prefix_sep .. o.prefix_sep })
  local dev = append_property(s, 'audio-device', { prefix = 'Device:' })
  local ao_mute = mp.get_property_native 'ao-mute' and ' (Muted)' or ''
  append_property(s, 'ao-volume', { prefix = 'AO Volume:', suffix = '%' .. ao_mute, nl = dev and '' or o.nl, indent = dev and o.prefix_sep .. o.prefix_sep })
  ...
  append(s, merge(r, ro, 'format'), { prefix = 'Format:', nl = cc and '' or o.nl, indent = cc and o.prefix_sep .. o.prefix_sep })
...
local function add_track(c, t, i)
  ...
  local flags = { 'default', 'forced', 'dependent', 'visual-impaired', 'hearing-impaired', 'image', 'albumart', 'external' }
  local any = false
  ...
  if append(c, t['codec-desc'], { prefix = 'Format:' }) then
    append(c, t['codec-profile'], { prefix = '[', nl = '', indent = ' ', prefix_sep = '', no_prefix_markup = true, suffix = ']' })
    if t['codec'] ~= t['decoder'] then
      append(c, t['decoder'], { prefix = '[', nl = '', indent = ' ', prefix_sep = '', no_prefix_markup = true, suffix = ']' })
    end
  end
  ...
  append(c, B(t['hls-bitrate']), { prefix = 'HLS Bitrate:', suffix = ' kbps', nl = bitrate and '' or o.nl, indent = bitrate and o.prefix_sep .. o.prefix_sep })
  append_resolution(c, { w = t['demux-w'], h = t['demux-h'], ['crop-x'] = t['demux-crop-x'], ['crop-y'] = t['demux-crop-y'], ['crop-w'] = t['demux-crop-w'], ['crop-h'] = t['demux-crop-h'] }, 'Resolution:')
  ...
  if track_rg then
    ...
    append(c, t['replaygain-track-gain'], { prefix = 'Gain:', suffix = ' dB', nl = '', indent = o.prefix_sep })
    append(c, t['replaygain-track-peak'], { prefix = 'Peak:', suffix = ' dB', nl = '', indent = o.prefix_sep })
  end
  if album_rg then
    append(c, '', { prefix = 'Album:', indent = o.indent .. o.prefix_sep, prefix_sep = '' })
    append(c, t['replaygain-album-gain'], { prefix = 'Gain:', suffix = ' dB', nl = '', indent = o.prefix_sep })
    append(c, t['replaygain-album-peak'], { prefix = 'Peak:', suffix = ' dB', nl = '', indent = o.prefix_sep })
  end
  if t['dolby-vision-profile'] or t['dolby-vision-level'] then
    ...
    append(c, t['dolby-vision-level'], { prefix = 'Level:', nl = '', indent = t['dolby-vision-profile'] and o.prefix_sep .. o.prefix_sep or '' })
  end
end
...
local function cache_stats()
  ...
  if not display_timer.oneshot and o.use_ass and o.plot_cache then
    r_graph = generate_graph(cache_ahead_buf, cache_ahead_buf.pos, cache_ahead_buf.len, cache_ahead_buf.max, nil, 0.8, 1)
  ...
  if not display_timer.oneshot and o.use_ass and o.plot_cache then
    speed_graph = generate_graph(cache_speed_buf, cache_speed_buf.pos, cache_speed_buf.len, cache_speed_buf.max, nil, 0.8, 1)
    speed_graph = o.prefix_sep .. speed_graph
  end
  append(stats, utils.format_bytes_humanized(speed) .. '/s', { prefix = 'Speed:', suffix = speed_graph })

  append(stats, utils.format_bytes_humanized(info['total-bytes']), { prefix = 'Total RAM:' })
  append(stats, utils.format_bytes_humanized(info['fw-bytes']), { prefix = 'Forward RAM:' })
  ...
  for n, range in ipairs(ranges) do
    append(stats, mp.format_time(range['start']) .. ' - ' .. mp.format_time(range['end']), { prefix = format('Range %s:', n) })
  end
...
local function print_page(page, after_scroll)
  ...
  if o.persistent_overlay then
    ...
    mp.osd_message((o.use_ass and ass_start or '') .. ass_content, display_timer.oneshot and o.duration or o.redraw_delay + 1)
  end
end
